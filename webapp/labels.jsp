...
<%@page import="com.bossky.ocean.ext.ResultPage"%>
...
<body>
	...
	<div class="comWidth clearfix mh500 mgt">
		...
		<c:if test="${!empty labelList}">
			<h3>
				结构式<span style="color: gray; font-weight: normal; font-size: 14px;">&nbsp;&nbsp;(提示：树形结构最多只显示4层)</span>
			</h3>
			...
			<div class="tree  clearfix">
				<c:forEach items="${labelList}" var="l">
					<c:if test="${null==l.parentId }">
						<div class="onetree">
							<a class="boot" href="themes.jspx?labelId=${l.id }"> <c:out
									value="${l.name}" />(${l.themeNum })
							</a>
							<c:if test="${null!=l.childList }">
								<ul class='child'>
									<c:forEach items="${l.childList }" var="c">
										<li><a class='boot' href='themes.jspx?labelId=${c.id }'>
												<div class='hxian'></div>${c.name}(${c.themeNum })</a> <c:if
												test="${null!=c.childList }">
												<ul class='child'>
													<c:forEach items="${c.childList }" var="cc">
														<li><a class='boot'
															href='themes.jspx?labelId=${cc.id }'>
																<div class='hxian'></div>${cc.name}(${cc.themeNum })</a> <c:if
																test="${null!=cc.childList }">
																<ul class='child'>
																	<c:forEach items="${cc.childList }" var="ccc">
																		<li><a class='boot'
																			href='themes.jspx?labelId=${ccc.id }'>
			...
			<div class="brick mgt">
				<c:forEach items="${labelList}" var="l">
					<a href="themes.jspx?labelId=${l.id }"><c:out
							value="${l.name}" />(${l.themeNum })</a>
				</c:forEach>
			</div>
		</c:if>
		<%--
