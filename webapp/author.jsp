	...
	<div class="comWidth clearfix mgt">
		...
		<div class="theam_r fr">
			<c:forEach items="${list}" var="t">
				<div class="art">
					<h3 class="art_title">
						<a href="/content.jspx?id=${t.id }" title="${t.title}"
							target="_blank"><c:out value="${t.title}" /></a>
					</h3>
					<p class="art_author">
						作者：
						<c:out value="${t.user.nickName }" />
						发表时间：
						<fmt:formatDate value="${t.createDate}"
					...
					<p class="art_btm clearfix">
						<span class="art_lab fl"> <c:forEach
								items="${t.labelList }" var="l" begin="0" end="2">
								<a href="themes.jspx?labelId=${l.id }" target="_blank"><c:out
										value="${l.name}" /></a>
							</c:forEach> <c:if test="${fn:length(t.labelList)>3}">
								<span style="line-height: 32px">...</span>
							</c:if>
						</span> <span class="art_read fr"> <span class="art_Num">浏览(${t.viewNum})&nbsp;
								评论(${t. commentsNum})&nbsp; 收藏(${t.collectNum })&nbsp;
								赞(${t.praiseNum })</span> <a href="/content.jspx?id=${t.id }"
